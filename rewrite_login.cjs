...
import { User, Lock, ShieldCheck, CheckCircle2, Factory, Car, Wrench } from 'lucide-react';
...
export default function SmartLogin({ onLogin }: SmartLoginProps) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (selectedRole === 'admin') {
      if (username === 'yacine' && password === 'yacine04') {
        onLogin('admin');
      } else {
        setError('بيانات الدخول غير صحيحة للمدير (yacine / yacine04)');
      }
    } else {
      if (username === 'ali' && password === 'ali123') {
        onLogin('worker');
      } else {
        setError('بيانات الدخول غير صحيحة للعامل (ali / ali123)');
      }
    }
  ...
  const roles = [
    { id: 'admin', label: 'الإدارة العليا', icon: ShieldCheck },
    { id: 'worker', label: 'الورشة', icon: Factory },
  ];
        ...
        >
          ...
          <div className="bg-white dark:bg-fbm-blue-card p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)] rounded-3xl border border-slate-200 dark:border-fbm-blue-border">
            <form className="space-y-6" onSubmit={handleLogin}>
              ...
              {/* Roles */}
              <div className="grid grid-cols-2 gap-3 mb-6 p-1.5 bg-slate-50 dark:bg-fbm-blue rounded-2xl border border-slate-200 dark:border-fbm-blue-border">
                {roles.map((role) => (
                  <button
                    key={role.id}
                    type="button"
                    onClick={() => setSelectedRole(role.id as any)}
                    className={\`flex items-center justify-center gap-2 py-3 px-2 rounded-xl text-sm font-bold transition-all \${
                      selectedRole === role.id
                        ? 'bg-white dark:bg-fbm-blue-card text-fbm-green shadow-sm ring-1 ring-black/5 dark:ring-white/5'
